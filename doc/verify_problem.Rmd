...
nsclc_info = nsclc_info |>
  dplyr::filter(sample %in% colnames(nsclc_peaks))
info_split_instrument = split(nsclc_info, nsclc_info$instrument)

nsclc_norms = purrr::map(info_split_instrument, function(in_instrument){
  tmp_norm = nsclc_norm[, in_instrument$sample]
  instrument_median = purrr::map_dfr(rownames(nsclc_norm), function(in_feature){
    ...
    tibble(feature = in_feature, sample_present = length(tmp_not0), sample_missing = length(tmp_data) - length(tmp_not0), median_all = med_all, median_present = med_not0)
  })
})
