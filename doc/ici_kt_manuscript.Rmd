...
```{r include_figure1, dn_id = figure_count$label_file("problem")}
...
Likewise, the *multiprocessing* python package makes it simple to distribute the sample-sample comparisons calculations across cores with very little overhead.
...
```{r compare_realistic, fig.width = 8, fig.height = 8, dn_id = figure_count$label_file("missing2cor")}
...
```{r left_censored_vs_random, fig.height = 8, fig.width = 8, dn_id = figure_count$label_file("leftcensored")}
...
```{r show_performance_graph, dn_id = figure_count$label_file("singleperf")}
...
`r figure_count$label_text("singleperf")`. Comparison of the Rcpp mergesort ici_kt, and base R's correlation function using the "pearson" and "kendall" methods as the number of features are increased from 100 to 5000.
...
```{r dependence_number, dn_id = figure_count$label_file("multiperformance")}
...
`r figure_count$label_text("multiperformance")`. Time taken by ICI-Kt as a function of the number of samples with multi-processing at the R level, using 80 cores.
...
```{r yeast_outlier_figure, dn_id = figure_count$label_file("yeast_outliers")}
yeast_single %>%
  add_method() %>%
...
We also note that these outliers seem to be a superset of the outliers noted by Gierlinski et al. (see `r supp_figure_count$label_text("yeast_outliers")` and `r supp_table_count$label_text("yeast_outliers")`).


```{r brainsonseq_figure, dn_id = figure_count$label_file("brainson_outliers")}
brainson_single %>%
  add_method() %>%
...
`r figure_count$label_text("brainson_outliers")` visualizes the sample-specific median of each metric calculated from the polycomb RNA RNA-Seq data set.
...
```{r adeno_figure, dn_id = figure_count$label_file("adeno_outliers")}
adeno_single %>%
  add_method() %>%
...
`r figure_count$label_text("adeno_outliers")` visualizes the sample-specific median metric value calculated from the TGCA RNA-Seq data set, with `r table_count$label_text("adeno_outliers")` listing the outliers detected from each metric.  
...
```{r load_ici_timings, fig.height = 8, fig.width = 5, dn_id = figure_count$label_file("icisubsampling")}
...
We note here that the theoretical assumption of linearity seems to be important for enabling the use of feature subsets.
In the case of Pearson correlation (see supp_figure_count$label_text("pearson_subsampling)), the differences in obtained correlation values are much, much larger, with differences decreasing much slower than ICI-Kt and Kendall-tau.
In fact, the differences in the Pearson correlation subset differences standard deviations never get as small as those obtained using via ICI-Kt and Kendall-tau.
...
```{r poor_pca_var, dn_id = figure_count$label_file("pcavardifference")}
...
```{r nonrandom_pca, dn_id = figure_count$label_file("pcaeval")}
...
RMF wrote the code in the ICIKendallTau R package, tested the ICI-Kt correlation code, and wrote all of the analysis code for this manuscript. PSB wrote the Python package. 
