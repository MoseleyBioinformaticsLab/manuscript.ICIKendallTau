...
Both dropping or imputing missing values are likely to cause the calculate sample-sample correlation values to deviate from the real sample-sample correlation values, especially with respect to specific data interpretation perspectives.
...
The virtual machine is running on top of a 50 node cluster, each with 4 10-core processors with hyperthreading, 3TB of RAM and an 8TB solid-state-drive array, provided by the Kentucky Research Informatics Cloud (KyRIC).
...
Grey points are all of the sample median correlations for each correlation method, colored if they were an outlier in any of the methods shown.
For a sample that is considered an outlier in any method, lines are drawn between methods.
...
This is due to overall there being a higher likelihood of pairwise samples being *complete* with the requirement for a feature to be in a greater fraction of a class of samples.
...
The rationale for this analysis is that stronger positive partial correlations should be reflected within known pathways versus between pathways.
