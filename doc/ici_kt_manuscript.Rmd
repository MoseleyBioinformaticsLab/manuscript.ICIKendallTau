...
With both simulated and real data sets from RNA-seq and metabolomics experiments, we demonstrate that the ICI-Kt allows for the inclusion of missing data values as interpretable information, which allows improved determination of outlier samples prior to differential analysis of omics datasets.
...
We examine the effect of missing values on various simulated data-sets, comparing the information-content-informed Kendall-tau (ICI-Kt) method with other simpler methods of handling the missing values, namely removing them or imputing them to zero.
...
Finally, we examine how feature-feature networks derived from various feature-feature correlations change depending on the correlation method used.
...
We do note that the calculations do **not require** this much compute resources; however, the VM was created for other compute requiring high memory resources, and was convenient to use for this work.
...
Counts were normalized using `DESeq2` with either the genotype or the growth environment as the grouping factor.
Samples were grouped by their genotype or growth environment for the calculation of median abundance across samples.
...
Counts were normalized using `DESeq2` with the sample genotype of *WT* or *Snf2* deletion as the experimental factor.
...
Counts were normalized using `DESeq2` with the tissue status as normal or tumor as the experimental factor.
...
Because the assigned peaks represent such a small amount of the overall data, we reprocess the full dataset to match peaks across samples.
...
Samples were grouped by the instrument (two different instruments were used to collect the data) and whether they were taken from normal or tumor tissue for the calculation of median abundance across samples.
Assigned peaks and un-assigned peaks were matched to each other by finding peaks within a 0.5 ppm cutoff.
We only considered peaks matched that had a single assigned and a single un-assigned peak match together.
...
For each matched peak, we used voting of the lipid categories and classes of the assignments to ascribe a single category and lipid class (where possible).

### Rat Stamina and Feeding Metabolomics Data Set
...
Metabolite annotations are based on Kyoto Encyclopedia of Genes and Genomes (KEGG) pathways that map from the provided KEGG compound IDs to various KEGG pathways.
Compound - pathway annotations were fetched using the kegg-pull python package [@huckvaleKegg_pullSoftwarePackage2023].

### Gene Annotations

Pathway annotations for RNA-Seq were obtained from Reactome pathways in the reactome.db Bioconductor package as well as organism specific annotation packages [@joshi-topeReactomeKnowledgebaseBiological2005; @ligtenbergReactomeDbSet2023].

### Number of Non-Missing Values and Median Values
...
For each of the datasets, we trimmed to features present in 25% or more of **any** of the sample classes (see above for the various classes of sampels in each dataset).
Various correlation measures are calculated between all remaining features.
For any given correlation, we generate the feature-feature network for that data set - correlation combination.
...
For each feature annotation (see Gene Annotations), we calculate three sums of the edge weights.

  1. The total sum of edge weights for all edges with features that are annotated to one or more of the annotations (total_sum).
...
The partitioning ratio (or q-ratio) is then:
...
In `r table_count$label_text("datasets")` we provide a summary of the number of features (measurements), samples, treatments or conditions, and the number of replicates per condition for each of the experimental datasets.

`r table_count$label_text("datasets")`. Number of features, samples, treatments, and replicates in each condition for each dataset.
```{r}
dataset_summary |>
  flextable() |>
...
In Do et al., the authors showed that there was a limit of detection effect, with a dependence on the day the samples were run.
However, one thing that seemed to be missing in the analysis was normalizing each sample for sample-to-sample variation. 
We feel that this is critical, given that even with Metabolon data, the standards do not correct for sample-to-sample variation.
...
For each of the datasets we investigated (three RNA-seq, one metabolomics, one lipidomics), for each experimental group of samples, we calculated the median abundance and number of present entries for each feature (gene or metabolite, see Methods).
As shown in `r figure_count$label_text("lod")`A for the yeast RNA-seq dataset (also see `r supp_figure_count$label_text(c("lod_adenocarcinoma", "lod_egfrgenotype", "lod_egfrgenotypetumorculture", 
"lod_nsclc", "lod_ratstamina"))` in Supplemental Materials for the other datasets), there is clearly a relationship between the median abundance and the number of present entries.
In fact, as the number of present values increases, there is clearly a minimum median value below which the values do not cross.
This relationship is extremely clear by calculating the minimum median value for each value of present features, and plotting them by number of present features, as shown in `r figure_count$label_text("lod")`B (see also `r supp_figure_count$label_text(c("lod_adenocarcinoma", "lod_egfrgenotype", "lod_egfrgenotypetumorculture", 
"lod_nsclc", "lod_ratstamina"))` in Supplemental Metarials for the other datasets).
Given this relationship of the minimum median value observed and the number of present features, we believe that the majority of missing values in many -omics datasets are due to being below the limit of detection.

This makes the ICI-Kt appropriate for use in many -omics datasets.
...
```{r performance, dn_id = figure_count}
...
single_core = s_perf_plot + inset_element(inset_perf, 0.1, 0.72, 1, 1)
multi_core = time_multi |>
  ...
  labs(x = "Number of Samples",
       y = "Log2(Time) (s)")
s_perf_plot + inset_element(inset_perf, 0.1, 0.72, 1, 1, ignore_tag = TRUE) + multi_core + plot_annotation(tag_levels = "A")
```
...
We analyzed datasets from RNA-Seq (3), lipidomics, and small molecule metabolomics for possible outliers using a variety of  different correlation and correlation-related metrics: (1) ICI-Kt; (2) ICI-Kt scaled by *completeness* (see Methods); (3) Pearson correlation; (4) Pearson using pairwise complete features; (5) Pearson on log(value + 1); (6) Pearson on log(value); and (7) Kendall-tau.
We primarily concentrate on the ICI-Kt, ICI-Kt * completeness, and Pearson log(x + 1) here, but all outliers by method are shown in the Supplemental materials (`r supp_figure_count$label_text(c("yeast_outliers", "adenocarcinoma_outliers"))`, `r supp_table_count$label_text(c("yeast_outliers", "egfrgenotype_outliers"))`).
Additionally, we compared how the outliers change if we require that a feature is present in at least a certain fraction of samples of a class with each different correlation measure (`r supp_figure_count$label_text(c("yeast_by_method", "adenocarcinoma_differences"))`).
...
`r figure_count$label_text("egfrgenotype_outliers")` visualizes the sample-specific median of each metric calculated from the Brainson RNA-Seq data set.
Likewise, `r table_count$label_text("egfrgenotype_outliers")` lists the outliers detected based on each metric.
...
```{r egfrgenotype_outliers, dn_id = figure_count}
compare_egfrgenotype_main = c("icikt", "icikt_complete", "pearson_log1p")
egfrgenotype_single %>%
  add_method() %>%
  dplyr::filter(which %in% compare_egfrgenotype_main) %>%
  ggplot(aes(x = sample_class, y = med_cor, color = outlier, group = sample_class)) +
  geom_sina() + 
  facet_wrap(~ method, nrow = 1) +
  labs(x = "Sample Class", y = "Median Correlation") +
...
`r figure_count$label_text("egfrgenotype_outliers")`. 
...
`r table_count$label_text("egfrgenotype_outliers")`.
...
egfrgenotype_table = compare_outlier_tables(egfrgenotype_single, compare_egfrgenotype_main, sort_var = "pearson_log1p")

if (!is.null(egfrgenotype_table)) {
  egfrgenotype_ft_out = egfrgenotype_table %>%
  set_nice_widths(sample_width = 1.5) %>%
  put_outlier_right()
  print(egfrgenotype_ft_out)
}
```

`r figure_count$label_text("adenocarcinoma_outliers")` visualizes the sample-specific median metric value calculated from the TCGA RNA-Seq data set, with `r table_count$label_text("adenocarcinoma_outliers")` listing the outliers detected from each metric.
...
```{r adenocarcinoma_outliers, dn_id = figure_count}
compare_adenocarcinoma_main = c("icikt", "icikt_complete", "pearson_log1p")
adenocarcinoma_single %>%
  add_method() %>%
  dplyr::filter(which %in% compare_adenocarcinoma_main) %>%
  ggplot(aes(x = sample_class, y = med_cor, color = outlier, group = sample_class)) +
  geom_sina() + 
  facet_wrap(~ method, nrow = 1) +
  labs(x = "Sample Class", y = "Median Correlation") +
...
`r figure_count$label_text("adenocarcinoma_outliers")`.
...
`r table_count$label_text("adenocarcinoma_outliers")`.
...
adenocarcinoma_table = compare_outlier_tables(adenocarcinoma_single, compare_adenocarcinoma_main, sort_var = "pearson_log1p")
...
### Generating Feature - Feature Networks
...
The Metabolomics Workbench data repository is supported by the Metabolomics Workbench/National Metabolomics Data Repository (NMDR) (grant# U2C-DK119886), Common Fund Data Ecosystem (CFDE) (grant# 3OT2OD030544) and Metabolomics Consortium Coordinating Center (M3C) (grant# 1U2C-DK119889).
We are heavily indebted to the University of Kentucky Center for Computational Sciences (CCS) provided Kentucky Research Informatics Cloud (KyRIC), an NSF supported computational resource for access to large virtual machines that allowed for methods development and the running of many of the larger datasets in this manuscript.
