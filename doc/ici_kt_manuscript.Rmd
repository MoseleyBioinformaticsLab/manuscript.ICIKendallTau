...
We analyzed datasets from RNA-Seq (3), lipidomics, and small molecule metabolomics for possible outliers using a variety of  different correlation and correlation-related metrics: (1) ICI-Kt; (2) ICI-Kt scaled by *completeness* (see Methods); (3) Kendall-tau; (4) Pearson correlation; (5) Pearson correlation with no zeros using pairwise complete features; (6) Pearson on log(value + 1); and (7) Pearson on log(value).
We primarily concentrate on the ICI-Kt, ICI-Kt * completeness, and Pearson log(x + 1) here for a few of the datasets, but all outliers by method are shown in the Supplemental materials (`r supp_figure_count$label_text(c("yeast_outliers", "egfrgenotype_outliers", "adenocarcinoma_outliers", "nsclc_outliers", "ratstamina_outliers",))`, `r supp_table_count$label_text(c("yeast_outliers", "egfrgenotype_outliers", "adenocarcinoma_outliers", "nsclc_outliers", "ratstamina_outliers",))`).
