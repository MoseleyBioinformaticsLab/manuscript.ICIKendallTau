...
egfr_med_cor = median_correlations(egfr_cor, egfr_info$type)
egfr_out_frac = outlier_fraction(t(log1p(egfr_counts)), egfr_info$type)

egfr_med_out = determine_outliers(egfr_med_cor, egfr_out_frac, frac_weight = 0)

egfr_complete_med = median_correlations(egfr_cor_complete, egfr_info$type)
...
egfr_pearson_med = median_correlations(egfr_pearson, egfr_info$type)
...
ggplot(egfr_all_out, aes(x = type, y = ))
...
`r paste_label("Figure ", figure_count, "f_adenooutliers")`. Median correlations for each sample to all other samples in the same group of either tumor or normal, using either ICI-Kt or Pearson correlation.
