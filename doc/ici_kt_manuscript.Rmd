...
The ICI-Kt correlation calculation is available in an R package on GitHub at https://moseleyBionformaticsLab/ICIKendallTau, as well as a Python module from PyPi under ICIKendallTau.
...
Of course some analytes will be missing at random just because of some issue with the instrument or that particular sample, but a large amount of missing measurements are left-censored due to analytes being below the effective detection limit of the instrument, as shown in `r paste_label("Figure ", figure_count, "f_problem")`.
...
`r paste_label("Figure ", figure_count, "f_problem")`. Graphical description of the left-censored data problem.
...
In a naive treatment of the left-centred missing data, if the values below the cutoff are set to missing, the data is log-transformed, and missing values subsequently set to 0; then the Kendall correlation will show some very odd patterns at low intensity cutoffs due to the introduction of discordant pairs.
