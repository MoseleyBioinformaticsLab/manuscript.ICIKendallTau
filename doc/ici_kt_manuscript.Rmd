...
In Do et al., the authors showed that there was a limit of detection effect, however it seemed mostly to be run-day dependent.
...
Unfortunately, the KORA4 metabolomics dataset is not publicly available, so we could not attempt to redo their analysis of missing values with the same dataset.
...
This relationship is extremely clear by pulling the minimum median value for each value of present features, and plotting them by number of present features, as shown in `r figure_count$label_text("lod")`B (see also XXX in Supplemental Metarials for the other datasets).
