...
`r paste_label("Figure", figure_count, "f_leftcensored")`. Effect of introducing missing values from a cutoff or randomly on different measures of correlation, including ICI-Kt, Kendall with pairwise complete, Kendall replacing missing with 0, Pearson with pairwise complete, and Pearson replacing missing with 0.
...
Outside of the differences between the subset and full set of correlations, we also evaluated each of the feature subsets using the correlation between loading fraction and principal component contributed variance, and the difference between the median loading fraction and the intended fraction.
...
`r paste_label("Figure", figure_count, "f_pcaeval")` shows that for both the PCA and variance informed subsampling methods, the loading fraction is related to the percent PC variance.
