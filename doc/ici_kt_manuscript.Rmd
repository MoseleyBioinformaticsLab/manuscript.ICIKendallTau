...
RNA-Seq dataset from null and knock-in EGFR mice mutants.
...
For each mutant, they were also grown in different ways and sequenced: (1) 2D plates; (2) 3D organioids; (3) cells sorted and selected using FACS; (4) total tumor without sorting.
**I think we still need more details from Brainson manuscript or presentation**.

### Yeast RNA-Seq Data Set

Summarized gene level counts were obtained from a GitHub project maintained by the Barton group [@christiancoleProfilingDifferentialGene2021].
...
supp_figure_count = increment_count(supp_figure_count, "supp_complexity")
```
...
In fact, regressing the time to number of samples using a formula with the expected algorithmic complexity, "pearson" shows O(n), ICI-Kt is O(nlog(n)), and "kendall" is O(n^2) (see `r paste_label("Figure S", supp_figure_count, "supp_complexity")`).
...
single_no_ici = dplyr::filter(single_core_perf, !(method %in% "ici_kt"))
...
outlier_fig_list = paste(
  paste_label("", figure_count, "yeast_outliers"),
  ", ",
  paste_label("", figure_count, "brainson_outliers"),
  " and ",
  paste_label("", figure_count, "adeno_outliers"),
  sep = ""
)
...
outlier_table_list = paste(
  ...
  ", ",
  paste_label("", table_count, "brainsonrnaseq_outliers"),
  " and ",
...
We analyzed three RNA-Seq datasets for possible outliers using a variety of  different correlation and correlation-related metrics: (1) ICI-Kt; (2) ICI-Kt scaled by *completeness* (see Methods); (3) Pearson correlation; (4) Pearson using pairwise complete features; (4) Pearson on log(value + 1); (5) Pearson on log(value); and (6) Kendall-tau.
We primarily concentrate on the ICI-Kt, ICI-Kt * completeness, and Pearson log(x + 1) here, but all outliers by method are shown in the Supplemental materials.
Additionally, we compared how the outliers change if we require that a feature is present in at least a certain fraction of samples of a class with each different correlation measure (supplemental materials).
...
`r paste_label("Figure ", figure_count, "yeast_outliers")`.
...
yeast_table = autofit(yeast_table)
yeast_table
```


`r paste_label("Figure ", figure_count, "yeast_outliers")` visualizes the sample-specific median of each correlation-related metric calculated from the Yeast RNA-Seq data set. 
...
We also note that these outliers seem to be a superset of the outliers noted by Gierlinski et al. (see `r paste_label("Figure S", supp_figure_count, "yeast_outliers")` and `r paste_label("Table S", supp_table_count, "yeast_outliers")`).


```{r brainsonseq_figure}
brainson_single %>%
  add_method() %>%
...
`r paste_label("Figure ", figure_count, "brainson_outliers")`. 
...
`r paste_label("Table ", table_count, "brainson_outliers")`.
...
```{r brainsonseq_table}
brainson_table = compare_outlier_tables(brainson_single, compare_brainson, sort_var = "pearson_log1p")

brainson_ft_out = autofit(brainson_table)
brainson_ft_out
```

`r paste_label("Figure ", figure_count, "brainson_outliers")` visualizes the sample-specific median of each metric calculated from the polycomb RNA RNA-Seq data set.
Likewise, `r paste_label("Table ", table_count, "brainson_outliers")` lists the outliers detected based on each metric.
...
`r paste_label("Figure ", figure_count, "adeno_outliers")`.
...
```{r adeno_table}
adeno_table = compare_outlier_tables(adeno_single, compare_adeno, sort_var = "pearson_log1p")
adeno_ft_out = autofit(adeno_table)
adeno_ft_out
```

`r paste_label("Figure ", figure_count, "adeno_outliers")` visualizes the sample-specific median metric value calculated from the TGCA RNA-Seq data set, with `r paste_label("Table ", table_count, "adeno_outliers")` listing the outliers detected from each metric.  
...
The wider spread of measurement completeness is expected, since TCGA is an aggregate of samples collected from various human cancer patents as compared to the first two examples which were collected from specific experiments in cell culture (`r paste_label("Table S", supp_table_count, "adeno_mad")`).
