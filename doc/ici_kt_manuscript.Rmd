...
yeast_data = compare_outlier_tables(yeast_outliers,
                                    c("icikt" = "ICI-Kt",
                                      "icikt_complete" = "ICI-Kt * Completeness",
                                      "pearson_log" = "Pearson"),
                                    "pearson_log")
yeast_outliers_plot = yeast_data$plot_data
yeast_outliers_table = yeast_data$table
```

```{r plot_yeast_outliers}
ggplot(yeast_outliers_plot, aes(x = sample_class, y = med_cor, group = sample_class, color = outlier)) + 
  geom_sina() +
...
yeast_outliers_table = set_caption(yeast_outliers_table,
...
yeast_outliers_table = autofit(yeast_outliers_table)
yeast_outliers_table
```



```{r outlier_polycomb_setup}
loadd(polycomb_outliers)
polycomb_data = compare_outlier_tables(polycomb_outliers,
                                       c("icikt" = "ICI-Kt",
                                      "icikt_complete" = "ICI-Kt * Completeness",
                                      "pearson_log" = "Pearson"),
                                    "pearson_log")

polycomb_outliers_plot = polycomb_data$plot_data
polycomb_outliers_table = polycomb_data$table
```

```{r plot_polycomb_outliers}
polycomb_outliers_plot = polycomb_outliers_plot %>%
  dplyr::mutate(observed_out = dplyr::case_when(
    grepl("outlier", sample_id) ~ TRUE,
    TRUE ~ FALSE
  ))
ggplot(polycomb_outliers_plot, aes(x = sample_class, y = med_cor, group = sample_class, color = observed_out, shape = outlier)) + 
  geom_sina(size = 2) +
  facet_wrap(~ method, scales = "free_y", nrow = 1) +
...
`r paste_label("Figure ", figure_count, "f_polycomboutliers")`.
Median correlations for each of the Polycomb RNA-Seq sample to all other samples in the same group, using either ICI-Kt, ICI-Kt * completeness, or Pearson correlation.
**Notice that the outliers detected by correlation do not correspond to outliers noted by the biochemists working with the tumors.**

```{r polycomb_outlier_table}
polycomb_outliers_table = set_caption(polycomb_outliers_table, caption = paste0(paste_label("Table ", table_count, "polycomb_outliers"), ". Polycomb dataset median correlation values and outlier determination for each outlier from each of the correlation methods."))
polycomb_outliers_table = autofit(polycomb_outliers_table)
polycomb_outliers_table
```



```{r outlier_adenocarcinoma_setup}
loadd(transcript_outliers)
transcript_data = compare_outlier_tables(transcript_outliers,
                                         c("icikt" = "ICI-Kt",
                                      "icikt_complete" = "ICI-Kt * Completeness",
                                      "pearson_log" = "Pearson"),
                                    "pearson_log")
transcript_outliers_plot = transcript_data$plot_data
transcript_outliers_table = transcript_data$table
```

```{r plot_adenocarcinoma}
ggplot(transcript_outliers_plot, aes(x = sample_class, y = med_cor)) + 
