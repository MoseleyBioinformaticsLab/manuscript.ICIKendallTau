...
However, one thing that seemed to be missing in their analysis was normalizing each sample for sample-to-sample variation. 
...
For each of the datasets we investigated (three RNA-seq and two metabolomics), for each experimental group of samples, we calculated the median abundance and number of present entries for each feature (gene or metabolite, see Methods).
As shown in `r figure_count$label_text("lod")`A for the yeast RNA-seq dataset (also see XXX in Supplemental Materials for the other datasets), there is clearly a relationship between the median abundance and the number of present entries.
...
This relationship is extremely clear by calculating the minimum median value for each value of present features, and plotting them by number of present features, as shown in `r figure_count$label_text("lod")`B (see also XXX in Supplemental Metarials for the other datasets).
