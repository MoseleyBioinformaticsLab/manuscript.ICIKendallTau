...
Correlation values calculated using Pearson correlation were calculated on log-transformed values in two ways: 
(1) just using the log-transformed values and leaving missing (NA) values as missing (Pearson);
(2) after log-transforming values, any missing (NA) were replaced with 0 (Pearson 0). This should be fine as the non-zero entries are all greater than 1.
For both Pearson methods, only features that are non-missing in both samples being compared are used for the correlation.
...
As shown in `r figure_count$label_text("pearson_nolog")`, as well as `r supp_figure_count$label_text("pearson_subsampling_nolog")` and `r supp_figure_count$count("pearson_sub_overrange_nolog")`, when the transcriptomics data is not log-transformed, Pearson correlation using subsets of features deviates wildly from those obtained using the full data set, much more than ICI-Kt or any of the other methods used.

```{r show_pearson_nolog, dn_id = figure_count$label_file("pearson_nolog")}
