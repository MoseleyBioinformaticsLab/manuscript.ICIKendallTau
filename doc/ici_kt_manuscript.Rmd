...
A negative analytical sample has values sorted in descending order.
...
```{r left_censored_vs_random}
loadd(left_censored_cor)

ggplot(left_censored_cor, aes(x = n_na, y = cor)) + 
  geom_point() +
  facet_wrap(~ which, nrow = 1)

loadd(random_censored_cor)

ggplot(random_censored_cor, aes(x = n_na, y = cor)) + 
  geom_point() +
  facet_wrap(~ which, nrow = 1)
```
...
As shown in Figure X, both of these feature selection methods perform extremely poorly, even with a fraction of 0.5.
...
Figure X. Sample-sample ICI-Kt correlations using all features compared with 1%, 10% and 30% random subsets of features. 
