...
### PolycombRNA RNA-Seq Data Set

RNA-Seq dataset from null and knock-in EGFR mice mutants. **going to need more than this**.

### Yeast RNA-Seq Data Set

Summarized gene level counts were obtained from a GitHub project maintained by the Barton group [@ProfilingDifferentialGene2021].
It should be noted that the data are also available from two figshare repositories [@SNF2KnockoutYeast2015; @WildtypeYeastGene2015].
...
The original outliers reported by Gierlinski et al are based on a combination of median correlations, feature outliers, and RNA-seq coverage.
...
figure_count = increment_count(figure_count, "f_polycomboutliers")
table_count = increment_count(table_count, "polycomb_outliers")
...
outlier_fig_list = paste(
  ...
  ", ",
  paste_label("", figure_count, "f_polycomboutliers"),
  " and ",
...
outlier_table_list = paste(
  ...
  ", ",
  paste_label("", table_count, "polycomb_outliers"),
  " and ",
...
In the case of Pearson correlation, the count values were log-transformed first using the *log* function (equivalent to log(value + 1)).
...
`r paste_label("Figure ", figure_count, "f_yeastoutliers")` visualizes the sample-specific median of each correlation-related metric calculated from the Yeast RNA-Seq data set. 
`r paste_label("Table ", table_count, "yeast_outliers")` lists the outliers detected based on each sample-specific median metric value.  
...
`r paste_label("Figure ", figure_count, "f_polycomboutliers")` visualizes the sample-specific median of each metric calculated from the polycomb RNA RNA-Seq data set.
Likewise, `r paste_label("Table ", table_count, "polycomb_outliers")` lists the outliers detected based on each metric.
