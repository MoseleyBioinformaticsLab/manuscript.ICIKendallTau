...
As shown in `r supp_figure_count$label_text(c("lod_adenocarcinoma", "lod_egfrgenotype", "lod_egfrgenotypetumorculture", "lod_typeandtumorculture", "lod_nsclc", "lod_ratstamina"))`, there is a direct relationship between the minimum median value and number of samples a feature is present in, unless the grouping of replicate samples is incorrect, as shown in `r supp_figure_count$label_text("lod_egfrgenotypetumorculture")`.
...
egfrgenotypetumorculture_single_plot = egfrgenotypetumorculture_single %>%
  add_method(other_method) %>%
  dplyr::filter(which %in% compare_egfrgenotype) %>%
  create_outlier_parallel_plot()
egfrgenotypetumorculture_single_plot
```
`r supp_figure_count$label_text("egfrgenotypetumorculture_outliers")`.

```{r}
#| label: egfrgenotypetumorculture_table
egfrgenotypetumorculture_table = compare_outlier_tables_bold(egfrgenotypetumorculture_single, compare_egfrgenotype, "pearson_log1p", map_method = other_method)
```

### EGFR RNASeq Genotype And Tumor Type

```{r}
#| label: setup_typeandtumorculture
all_typeandtumorculture_outliers = rbind_outliers(typeandtumorculture_outliers) |>
  perc_to_number()
```

```{r}
#| label: typeandtumorculture_outliers
typeandtumorculture_single_plot = typeandtumorculture_single %>%
  add_method(other_method) %>%
  dplyr::filter(which %in% compare_egfrgenotype) %>%
  create_outlier_parallel_plot()
typeandtumorculture_single_plot
```
`r supp_figure_count$label_text("typeandtumorculture_outliers")`.

```{r}
#| label: typeandtumorculture_table
typeandtumorculture_table = compare_outlier_tables_bold(typeandtumorculture_single, compare_egfrgenotype, "pearson_log1p", map_method = other_method)
```
...
Here, we explore what happens if we increase the number of samples required for a gene to be present in at 25% and 50% of a class for the yeast samples and adenocarcinoma samples, and 25%, 50%, 75% and all samples of a class for the EGFR samples.
...
nsclc_diffs = calculate_differences(nsclc_summary)
...
ratstamina_diffs = calculate_differences(ratstamina_summary)
...
save(yeast_table, egfrgenotype_table, egfrgenotypetumorculture_table, typeandtumorculture_table, adenocarcinoma_table, nsclc_table, ratstamina_table, file = here::here("doc/supp_stuff.rda"))
```
