...
```{r compare_simple, counter_identifier = supp_figure_count$label_file("kt_pearson")}
loadd(positive_kt)
...
ggplot(compare_df, aes(x = pearson, y = ici_kt)) + 
  geom_point() +
...
```{r ici_full_distribution, counter_identifier = supp_figure_count$label_file("ici_distribution")}
loadd(all_kt)
...
```{r kt_full_distribution, counter_identifier = supp_figure_count$label_file("kt_distribution")}
...
```{r plot_complexity, , counter_identifier = supp_figure_count$label_file("complexity")}
...
```{r yeast_single_plot, counter_identifier = supp_figure_count$label_file("yeast_outliers")}
yeast_single2 %>%
  add_method(map_method = c("icikt" = "IK",
...
```{r show_brainson, counter_identifier = supp_figure_count$label_file("brainson_outliers")}
brainson_single %>%
  add_method() %>%
...
```{r show_adeno_outliers, counter_identifier = supp_figure_count$label_file("adeno_outliers")}
adeno_single %>%
  add_method() %>%
...
```{r yeast_by_method, counter_identifier = supp_figure_count$label_file("yeast_by_method")}
...
```{r yeast_by_fraction, counter_identifier = supp_figure_count$label_file("yeast_by_keep_num")}
ggplot(all_yeast, aes(x = sample_class, y = med_cor, color = outlier, group = sample_class)) +
  geom_sina() +
...
```{r yeast_plot_differences, counter_identifier = supp_figure_count$label_file("yeast_differences")}
...
```{r brainson_plot1, , counter_identifier = supp_figure_count$label_file("brainson_by_method")}
...
```{r brainson_plot2, counter_identifier = supp_figure_count$label_file("brainson_by_keep_num")}
ggplot(all_brainson_method, aes(x = sample_class, y = med_cor, color = outlier, group = sample_class)) + 
  geom_sina() +
  facet_grid(method ~ keep_num)
```
...
```{r brainson_plot_differences, counter_identifier = supp_figure_count$label_file("brainson_differences")}
...
```{r adeno_by_method, counter_identifier = supp_figure_count$label_file("adeno_by_method")}
...
```{r adeno_by_fraction, counter_identifier = supp_figure_count$label_file("adeno_by_keep_num")}
ggplot(adeno_method, aes(x = sample_class, y = med_cor, color = outlier, group = sample_class)) +
  geom_sina() +
  facet_grid(method ~ keep_num)
```
...
```{r adeno_plot_differences, counter_identifier = supp_figure_count$label_file("adeno_differences")}
