...
pearson_diff = data.frame(ref = extract_data(ref_pearson, "cor"),
                       f_01 = 
...
combined_random_pearson$method = "Pearson"
combined_random_kendall$method = "Kendall"

all_combined = rbind(combined_random, combined_random_pearson, combined_random_kendall)

use_colors = scale_color_discrete()$palette(3)
names(use_colors) = c("ICI-Kt", "Kendall", "Pearson")
compare_plot = 
...
ici_kendall_values = all_combined %>%
  dplyr::filter(!(method %in% "Pearson"))
ici_compare_kendall = 
  (ggplot(ici_kendall_values, aes(x = fraction, y = median, color = method)) +
     geom_line() +
     scale_color_manual(values = use_colors) +
     labs(x = "Fraction of Features", y = "") +
     theme(legend.position = "none")) /
  (ggplot(ici_kendall_values, aes(x = fraction, y = sd, color = method)) +
     geom_line() +
     scale_color_manual(values = use_colors) +
     labs(x = "Fraction of Features", y = "") +
     theme(legend.position = "none"))

compare_plot | ici_compare_kendall
```
...
Left: all three methods.
Right: ICI-Kt and Kendall correlations only.
