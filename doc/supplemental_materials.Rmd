...
supp_figure_count = c("_" = 0)
supp_table_count = c("_" = 0)
...
```{r compare_simple}
supp_figure_count = increment_count(supp_figure_count, "kt_pearson")
supp_figure_count = increment_count(supp_figure_count, "ici_distribution")
supp_figure_count = increment_count(supp_figure_count, "kt_distribution")
...
`r paste_label("Figure S", supp_figure_count, "kt_pearson")`. Comparison of ICI-Kt and Pearson correlations.
...
These distributions are shown in `r paste_label("Figure S", supp_figure_count, "kt_distribution")`.
...
`r paste_label("Figure S", supp_figure_count, "ici_distribution")`.
...
`r paste_label("Figure S", supp_figure_count, "kt_distribution")`.
...
The run times and fitted lines for each method (R's Pearson, the ICI-Kt, and R's Kendall-tau) are shown in `r paste_label("Figure S", supp_figure_count, "supp_complexity")`.
...
`r paste_label("Figure S", supp_figure_count, "supp_complexity")`. Time in seconds needed as a function of the number of features, with a fitted line for the assumed complexity for each of the methods tested, including R's Pearson correlation, the ICI-Kt mergesort, and R's Kendall-tau correlation algorithm.
...
supp_figure_count = increment_count(supp_figure_count, "yeast_outliers")
...
`r paste_label("Figure S", supp_figure_count, "yeast_outliers")`.
...
supp_table_count = increment_count(supp_table_count, "yeast_outliers")
```

`r paste_label("Table S", supp_table_count, "yeast_outliers")`.
...
supp_figure_count = increment_count(supp_figure_count, "brainson_outliers")
...
`r paste_label("Figure S", supp_figure_count, "brainson_outliers")` shows the effect of including the pairwise completeness measure, where the "total" tumor samples have a higher correlation than the "sorted" samples where a specific subset of cells were collected from each sample and sequenced.
This is due to having more genes with non-zero reads in general over the sorted samples.


`r paste_label("Table S", supp_table_count, "brainson_outliers")` indicates the outliers determined with each correlation method.
...
```{r adeno_load}
supp_figure_count = increment_count(supp_figure_count, "adeno_outliers")
...
`r paste_label("Figure S", supp_figure_count, "adeno_outliers")`.
TCGA Adenocarcinoma outliers using various measures of correlation.

```{r adeno_table}

supp_table_count = increment_count(supp_table_count, "adeno_outliers")
```

```{r summarize_adeno}
supp_table_count = increment_count(supp_table_count, "adeno_mad")
...
`r paste_label("Table S", supp_table_count, "adeno_mad")`.
...
As `r paste_label("Figure S", supp_figure_count, "adeno_outliers")` and `r paste_label("Table S", supp_table_count, "adeno_outliers")`, although there are not many outliers in these relatively large groups, Pearson correlation using log-tansformed values results in the most outliers.
...
`r paste_label("Figure S", supp_figure_count, "yeast_differences")`.
...
```{r brainson_plot1}
supp_figure_count = increment_count(supp_figure_count, "brainson_by_method")
...
`r paste_label("Figure S", supp_figure_count, "brainson_by_method")`.
Median correlations by correlation method and applying different fractional cutoffs for Brainson RNA-seq data.


```{r brainson_plot2}
supp_figure_count = increment_count(supp_figure_count, "brainson_by_keep_num")
ggplot(all_brainson_method, aes(x = sample_class, y = med_cor, color = outlier, group = sample_class)) + 
  geom_sina() +
  facet_grid(method ~ keep_num)
```

`r paste_label("Figure S", supp_figure_count, "brainson_by_keep_num")`.
Median correlations by correlation method and applying different fractional cutoffs for Brainson RNA-seq data.


```{r brainson_plot_differences}
supp_figure_count = increment_count(supp_figure_count, "brainson_differences")
...
`r paste_label("Figure S", supp_figure_count, "brainson_differences")`.
...
supp_figure_count = increment_count(supp_figure_count, "adeno_by_method")
ggplot(adeno_method, aes(x = sample_class, y = med_cor, color = outlier, group = sample_class)) + 
  geom_sina() +
  facet_grid(keep_num ~ method)
```

`r paste_label("Figure S", supp_figure_count, "adeno_by_method")`. 
Median correlations by correlation method and applying different fractional cutoffs for TCGA adenocarcinoma RNA-seq data.

```{r adeno_by_fraction}
supp_figure_count = increment_count(supp_figure_count, "adeno_by_keep_num")
ggplot(adeno_method, aes(x = sample_class, y = med_cor, color = outlier, group = sample_class)) +
  geom_sina() +
  facet_grid(method ~ keep_num)
```

`r paste_label("Figure S", supp_figure_count, "adeno_by_keep_num")`. 
Median correlations by correlation method and applying different fractional cutoffs for TCGA adenocarcinoma RNA-seq data.

```{r adeno_plot_differences}
supp_figure_count = increment_count(supp_figure_count, "adeno_differences")
...
`r paste_label("Figure S", supp_figure_count, "adeno_differences")`. 
