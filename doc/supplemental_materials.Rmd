...
nsclc_method = add_method(all_nsclc_outliers, other_method) %>%
  clear_outliers()
ggplot(nsclc_method, aes(x = sample_class, y = med_cor, color = outlier, group = sample_class)) + 
  geom_sina() +
  facet_grid(keep_num ~ method) +
  theme(axis.text.x = element_text(angle = 90)) +
  labs(x = "Sample Class", y = "Median Correlation")
...
ratstamina_method = add_method(all_ratstamina_outliers, other_method) %>%
  clear_outliers()
ggplot(ratstamina_method, aes(x = sample_class, y = med_cor, color = outlier, group = sample_class)) + 
  geom_sina() +
  facet_grid(keep_num ~ method) +
  theme(axis.text.x = element_text(angle = 90)) +
  labs(x = "Sample Class", y = "Median Correlation")
