...
For outlier detection, median sample-sample correlations within the sample class (genotype, condition, etc) is calculated, and $log(1 - cor_{median})$ calculated to transform it into a score, and then outliers determined using *grDevices::boxplot.stats*, which by default are at 1.5X the whiskers in a box-and-whisker plot.
