...
We created a dataset of with `r lod_vars$n_feature` features and `r lod_vars$n_sample` samples, starting with a sample from a random log-normal distribution with a mean-log value of `r lod_vars$meanlog`, and standard deviation of `r lod_vars$sdlog`.
...
Three different correlation methods were used, ICI-Kt, Kendall-tau and Pearson correlation after imputing all missing values with $\frac{1}{2}$ the lowest observed value in the dataset after censoring.
Differences between the reference and trimmed correlations (with global imputation for Kendall-tau Pearson) are calculated, as well as the difference in the absolute value of differences of ICI-Kt and Pearson imputed.
...
We created a set of `r lod_vars$n_sample` fake samples with uniform noise on the log-scale, with relatively constant dynamic ranges, and introduced changes to the overall dynamic range by random values from a uniform distribution across the samples, at varying levels (see Methods).
...
Based on the number of values being censored, limits of 0.5, 1, and 1.5 were selected.
...
Low, med, and high indicate the level of changes in dynamic range, using 0.5, 1, and 1.5, respectively.
...
Kendall-tau using the imputed values resulted in identical correlations as ICI-Kt (see `r supp_figure_count$label_text("icikt-ktimputed")`), as was also observed when comparing the effects of left-censoring vs randomly missing data.
