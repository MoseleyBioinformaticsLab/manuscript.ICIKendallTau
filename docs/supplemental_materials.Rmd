...
#| fig-width: 12
#| fig-height: 8
ref_tmp_pos = realistic_reference_plot$positive + theme(axis.text = element_text(size = 10))
ref_tmp_neg = realistic_reference_plot$negative + theme(axis.text = element_text(size = 10))
...
```{r icikt-ktimputed, dn_id = supp_figure_count}
vl_icikt_kt_graph
```

`r supp_figure_count$label_text("icikt-ktimputed")`.
Correlations between 100 samples with varying amounts of missingness as a function of changing the dynamic range of the values by increasing the lower limit of detection, using ICI-Kt, or Kendall-tau after imputing missing values with ½ the lowest value in the dataset.
